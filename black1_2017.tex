...
associated with the single unit eigenvalue of $A$. The condition
 that the remaining eigenvalues of $A$ are less than unity
...
$E(x_t - E_0 x_t)(x_t - E_0 x_t)'$ | $\sum_{h=0}^{t-1} A^h C C' (A^h)'$ \cr
stationary mean | $(I - A)\mu =0$ \nr
stationary variance | $\Sigma_\infty = A \Sigma_\infty A' + C C' $ \nr
...
the state $x_t$ governed by our linear stochastic state space system $A, C, G$.
...
For $t \geq 0$, let $y^t = [y_t, y_{t-1}, \ldots, y_0]$.  We want seek an expression for the probability  distribution of $y_t$ conditional on 
history 
$y^{t-1}$ that has a convenient recursive representation. The Kalman filter attains that by constructing recursive formulas for objects $\hat x_t, \Sigma_t$
...
At each date,
our approach is to regresse
what we don't know on what we know. Let's start at date $t=0$. We arrive at date $0$ knowing $\hat x_0, \Sigma_0$. Then we observe $y_0$ and make
inferences.  It will turn out that among the objects with which  we leave time $t = 0$ will be $\hat x_1, \Sigma_1$.  This gives a perspective from which 
 ``we are in the same situation at the start of period $1$ that we were at the start of period $0$'', an insight that activates a recursion.
...
by the time $0$ version of equation  \Ep{kalf2}.    We form  revised beliefs about the mean of $x_0$ after observing
$y_0$ by computing the  distribution of $x_0$ conditional on $y_0$.  The conditional  mean $E [x_0| y_0] =  \hat x_0 + L_0 (y_0 - G \hat x_0)$ 
satisfies 
 the appropriate version of the population regression  formula \Ep{leastsq12}, namely, 
...
Having constructed $E [x_0 | y_0]$, we can  construct $\hat x_1 = E [x_1 | y_0]$ as follows.\NFootnote{It is understood that we know $\hat x_0$.  Instead of writing
...
that $E[x_1 | \hat x_0] = A\hat x_0$ and that
...
Thus, we have deduced  the conditional distribution  $x_1 | y_0 \sim{\cal N}(\hat x_1, \Sigma_1)$.
...
It is appropriate to view system \Ep{kalf1000} as a mapping a mean, covariance pair  $(\hat x_0, \Sigma_0)$ into
...
Equation \Ep{kalf10new} is known as a matrix Riccati difference equation that restricts a sequence of  
...
basis for the information set $y_{t-1}, y_{t-2}, \ldots, y_0$.  Instead of computing $E [x_t | y_{t-1}, y_{t-2}, \ldots, y_0]$ as
one big least squares regression, the Gram-Schmidt process computes a sequence of much smaller regressions on successive components
...
System  \Ep{kalf1}, \Ep{kalf2}, \Ep{kalf3} is an example of a hidden Markov model.  The stochastic process $\{y_t\}_{t=0}^\infty$ of observables is not Markov, but the hidden
process $\{x_t\}_{t=0}^\infty$ is Markov, and so is the process $\{\hat x_t, \Sigma_t\}$ that constitutes sufficient statistics
for the  probability distributions of   $y_t$ conditional on $[ y_{t-1}, y_{t-2}, \ldots, y_0]$.
